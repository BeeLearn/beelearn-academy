<script setup lang="ts">
    ...
    const viewTypes = computed(() => topics.value.flatMap((topic) => {
        const questionViewTypes = topic.topic_questions.map<[ViewType, TopicQuestion | Topic]>(topic_question => {
    ...
    const onQuestionSubmit = async function(){
        ...
        try {
            const [_viewType, value] = viewTypes.value[page.value];
            const topicQuestion = value as TopicQuestion;
            ...
            if(questionRef.value!.isCorrect){        
                ...
                topicStore.setTopicQuestion(topicQuestion.topic!, response.data);
                page.value ++;
            } else {
    ...
    <section class="fixed inset-0 flex flex-col bg-white z-10">
        ...
        <template v-else>
            <div
                ...
                <div 
                    class="order-3 flex-1 flex flex-col space-y-4 col-span-100 row-span-100 px-4 overflow-y-scroll"
                        ...
                        <Transition enter-active-class="animate-slide-in-right animate-duration-200">
                            <div
                                ...
                                <TopicDynamicQuestion
                                    :topic-question="value as TopicQuestion"
            ...
            <TopicFooter
                ...
                @continue="async () => {
                    if(questionRef)
                        ...
                        if(page < viewTypes.length - 1) page++;
