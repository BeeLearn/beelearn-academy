...
const onQuestionSubmit = async function () {
  ...
  try {
    ...
    snackbar.show(
      ...
      e("p", { textContent: "An unexpected error occur. Try refreshing page!"})
    );
  } finally {
...
<template>
  <section class="fixed inset-0 flex flex-col bg-white z-10">
    <div v-if="['idle', 'pending'].includes(loadingState)" class="m-auto progress progress-primary" />
    <template v-else>
      <div class="flex-1 grid grid-cols-2 grid-row-1 overflow-y-scroll" md="flex p-4">
        <div class="order-first col-start-1 col-end-3 py-4" md="flex-1 p-0">
          <div class="flex items-center space-x-2">
            <button class="p-4" @click="$router.back()">
              <UnoIcon class="i-mdi:close text-xl" />
            </button>
            <p class="text-nowrap truncate">{{ breadcrumb?.lesson.name }}</p>
          </div>
        </div>
        <TopicThreadAction v-if="topic" :topic="topic" />
        <div class="order-3 flex-1 flex flex-col space-y-4 col-span-100 row-span-100 px-6 overflow-y-scroll"
          md="order-2 flex-none w-md py-4" lg="w-2xl">
          <div class="flex items-center space-x-6">
            <StepProgressBar :progress="currentProgress" />
            <div class="w-1/3 flex" md="place-content-end">
          ...
          <template v-for="[viewType, value], index in viewTypes" :key="value.id">
            <Transition enter-active-class="animate-slide-in-right animate-duration-200">
              <div v-if="viewType === ViewType.questionView && index === page"
                ...
                <TopicDynamicQuestion :topic-question="value" v-model:inner-ref="questionRef" v-model:changed="isChanged"
                  v-model:submitting="isSubmitting" @submit="onQuestionSubmit" />
              </div>
              <TopicDynamicContent v-else-if="viewType === ViewType.topicView && index === page" :topic="value" />
            </Transition>
          </template>
        </div>
      </div>
      <TopicFooter :loading="isLoading" :can-back="page > 0" :is-question-view="Boolean(questionRef)"
        :action-disabled="!isChanged && Boolean(questionRef)"
        :topic-question="(viewTypes.at(page)?.at(1) as TopicQuestion | undefined)" @next="page++" @back="page--"
        @continue="async () => {
          if (questionRef)
      ...
      <Teleport to="#__nuxt">
        <LessonCompleteDialog :visible="isCompleted" :lessonName="breadcrumb!.lesson.name" />
      </Teleport>
    </template>
    <NuxtPage />
  </section>
